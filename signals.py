...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    entry_base = None
    if actionable:
        breakout_bias = bool(impulse_legitimacy >= 0.72 or ((impulse_type_hint in ("ORB", "VWAP")) and impulse_legitimacy >= 0.62))
        pullback_bias = bool((not breakout_bias) and accept_ok and (stage == "CONFIRMED" or impulse_legitimacy < 0.62))
        if near_pullback and not near_break:
            choose_pullback = True
        elif near_break and not near_pullback:
            choose_pullback = False
        elif breakout_bias and not stale_breakout and dist_br <= (1.15 * max(prox_dist, 1e-9)):
            choose_pullback = False
        elif pullback_bias and dist_pb_band <= (0.90 * dist_br):
            choose_pullback = True
        else:
            choose_pullback = bool(near_pullback and dist_pb_band <= dist_br and impulse_legitimacy < 0.80)
